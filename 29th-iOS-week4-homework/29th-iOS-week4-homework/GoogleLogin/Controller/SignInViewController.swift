...
class SignInViewController: UIViewController, UITextFieldDelegate {
  ...
  @IBAction func touchUpSignUpButton(_ sender: Any) {
    guard let nextVC = storyboard?.instantiateViewController(withIdentifier: Identifiers.signUpVC) as? SignUpViewController
    else { return }
  ...
  func requestLogin() {
    UserSignService.shared.login(email: emailTextField.text ?? "" ,
      ...
      case .success(let loginResponse):
        guard let response = loginResponse as? AuthResponseData else { return }
